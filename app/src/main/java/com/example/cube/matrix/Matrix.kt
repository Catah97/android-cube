...
/**
 * Source: https://github.com/yinpeng/kotlin-matrix/blob/master/src/main/kotlin/com/ichipsea/kotlin/matrix/Matrix.kt
 */
...
fun <T> vectorOf(vararg elements: T): Matrix<T> {
    return ListMatrix(elements.size, 1, elements.asList())
}
...
@Suppress("NON_PUBLIC_CALL_FROM_PUBLIC_INLINE")
inline fun createZeroMatrix(cols: Int, rows: Int): Matrix<Int> {
    return createMatrix(cols, rows) { _, _ -> 0 }
}
